...
def modified_run_person_clustering_experiment( # Renamed for clarity, or keep your name
        ...
        df_all_data,
        needed_count,image_cluster_dict
):
    ...
    for image_id in input_images_for_category:
        img_info_row = df_all_data[df_all_data['image_id'] == image_id]
    ...
    if len(images_for_mlb) < 2:  # Need at least two images with person data to compare
        result = select_by_cluster(relevant_cluster_labels, image_cluster_dict)
    ...
    for cluster_label, images_in_cluster in person_clusters_dict.items():
        ...
        images_ordered = sorted(
            images_in_cluster,
            key=lambda img_id: df_all_data.set_index('image_id').loc[img_id, 'total_score'], reverse=True
        )
